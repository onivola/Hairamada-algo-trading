...
def checkIntervalPrice(buy,sell,interval):
	...
	buy= float(buy)
	sell= float(sell)
	if(sell>price[0]+0 and sell<price[0]+20  and buy>price[0]-20 and buy<price[0]-0): #price
...
def CheckQuantite(buy,sell):
	if(len(buy)>0 and len(sell)>0):
		...
		Price = float(price[0])
		if(float(buy[1])>float(sell[1]) and float(buy[1])>=0 and float(sell[1])<0.04): #quantite
			return [2,float(sell[0])] #miakatra
		elif(float(buy[1])<float(sell[1]) and float(sell[1])>=0 and float(buy[1])<0.04):
...
#img = pyautogui.screenshot('gain.png')
