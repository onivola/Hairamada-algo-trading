        ...
        if(ibx==-1): #not in list
            ...
            if(Asup==1):
                ...
                if(Bey>Bby and Bby!=0):
                    ...
                    return False
            if(BY-AY<=15 and BY-AY>=-15 and Asup<1):
                print("df="+str((BY)-(AY)))
                Abx=AX
                Bbx=BX
                Aby=AY
...
def SymbolScreem(symbol,xb,xe,yb,ye,BUY,SELL,sleep):
    ...
    while 1:
        ...
        if(len(ListBleuX)>0 and len(ListBleuY)>0 and len(ListPinkX)>0 and len(ListPinkY)>0):
            PinkBleau = CheckAcross(ListPinkX,ListPinkY,ListBleuX,ListBleuY,5)
...
symbolThread1 = threading.Thread(target=SymbolScreem,args=("EURUSD",419,1187,177,686,[1277,545],[1277,606],60)) #hp 1 
