...
//--- delete the indicator handle and deallocate the memory space it occupies
   ...
   IndicatorRelease(iMA55_handle);
   IndicatorRelease(iMA5_handle);
...
//---
   ...
   double x_max=iRSI_buf[ArrayMaximum(iRSI_buf)]; //maximum value over the range
     ...
     double x_min55=iMA55_buf[ArrayMinimum(iMA55_buf)]; //minimum value over the range
   double x_max55=iMA55_buf[ArrayMaximum(iMA55_buf)]; //maximum value over the range
   
     double x_min5=iMA5_buf[ArrayMinimum(iMA5_buf)]; //minimum value over the range
   double x_max5=iMA5_buf[ArrayMaximum(iMA5_buf)]; //maximum value over the range
...
//--- In the loop, fill in the array of inputs with the pre-normalized indicator values
   ...
   int i = 0;
    ...
    while(i<10)
     {
      inputs[j]=(((iMA55_buf[i]-x_min55)*(d2-d1))/(x_max55-x_min55))+d1;
      i = i + 1;
      j = j+1;
     }
     ...
     i=0;
      while(i<10)
     {
      inputs[j]=(((iMA5_buf[i]-x_min5)*(d2-d1))/(x_max5-x_min5))+d1;
      i = i + 1;
      j = j+1;
     }
