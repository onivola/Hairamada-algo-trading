...
void OnTick(){
   ...
   double lastdiff = MathAbs(Current-chandel[1]);
   Print("Current"+(string)Current);
   Print("chandel"+(string)chandel[1]);
   Print("diffffff = "+result[0]);
   Print("DIIIIIIIF="+(string)lastdiff);
   double CheckMA = CheckMAinPrice();
   if(posTotal <= 0 && lastdiff >=1.5 && result[0]<=1.5) {
      interupt=true;
      //open(_Symbol);
      BuyPosition();
      tmp=0;
   }
   else if (posTotal > 0 && interupt==true && time>=59){
