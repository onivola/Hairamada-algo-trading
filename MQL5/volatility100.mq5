...
void OnTick(){
  ...
  Print("bands===="+LastBands);
   ...
   if(posTotal<=0) {
      ...
      /*if(ao1==0 && ao2==0 && ao3==0 && adx==0 && time>=0 && time<=10 && LastBands==0) {//3ao 1adx
         SellPosition();
         posNum=1;
      }*/
      if(ao1==0 && ao2==0 && ao3==0 && ao4==0 && ao5==0 && time<=10) { //4ao
         SellPosition();
         posNum=3;
      }

      else if(ao1==1 && ao2==1 && ao3==1 && ao4==1 && ao5==1 && time<=10) { //4ao
         BuyPosition();
         posNum=5;
      }
...
void takeProfit() {
 ...
 int posTotal = getSymbolPositionTotal(_Symbol);
     ...
     if(posTotal>0 && (posNum==5 || posNum==3)) {
   ...
   //Print("++AO2M="+ao2);
    ...
    //Print("++AOM3="+ao3);
     ...
     double ao5 = CheckAO(PERIOD_M5,1);
      ...
      Print("posNum="+posNum);
double time = Seconds();
         if(posTotal>0 && time>=58) {
            close(_Symbol);
            posNum=0;
         }
         if(posTotal>0 && posNum==5 && (ao1==0 || ao2==0 || ao3==0 || ao4==0 || ao5==0)) {
            close(_Symbol);
            posNum=0;
         }
          if(posTotal>0 && posNum==3 && (ao1==1 || ao2==1 || ao3==1 || ao4==1 || ao5==1)) {
            close(_Symbol);
            posNum=0;
         }
         /*if(posTotal>0 && posNum==4 && (profit<=-3 ||  ao3==0)) {
            close(_Symbol);
            posNum=0;
         }
          if(posTotal>0 && posNum==1 && (profit<=-3 || ao3==1)) {
            close(_Symbol);
            posNum=0;
         }*/
      }
