...
void OnTick(){
  ...
  Print("bands===="+LastBands);
   ...
   Print("rsi===="+rsi);
   if(posTotal<=0) {
      if(rsi==0 && LastBands==0 && ao3==0 && ao4==0 & ao5==0) { //4ao
         SellPosition();
         posNum=3;
      }
      else if( rsi==1 && LastBands==1 &&  ao3==1  && ao4==1 && ao5==1) { //4ao
         BuyPosition(); 
         posNum=5;
      }
   }
...
void takeProfit() {
 ...
 int posTotal = getSymbolPositionTotal(_Symbol);
     ...
     if(posTotal>0 && posNum!=2 && posNum!=6) {
         ...
         if(posTotal>=0 && posNum==3 && (ao3==1)) { //sell ao4
            close(_Symbol);
            posNum=0;
         }
          if(posTotal>=0 && posNum==5  && (ao3==0)) { //buy ao4
            close(_Symbol);
            posNum=0;
         }
