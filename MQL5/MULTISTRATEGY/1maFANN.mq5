...
//+------------------------------------------------------------------+
void NormalizePriceOnly(double& xy[])
{
   /*int max = ArrayMaximum(xy,4,0);
   int min = ArrayMinimum(xy,4,0);
   double MIN = xy[min];
   double MAX = xy[max];*/
...
void Get_XY(double& MA[],double& MA55[],double& RSI[],double& XY[],double& iy[]) {
   ...
   Print("ma");
   Print(MA[0]);
   for(int i=21;i<ArraySize(MA)-351;i++) {
            if(MA55[i]>=MA[i] && MA55[i+1]<MA[i+1]) {
...
void fit() {
  ...
  double MA55[];
  double RSI[];
  GetiRSIArray(day*1441,PERIOD_M5,30,RSI);
  GetiMAArray2(PERIOD_M5,1,0,MODE_SMA,PRICE_CLOSE,day*1441,MA);
  int max = ArrayMaximum(MA,4,0);
   int min = ArrayMinimum(MA,4,0);
   MIN = MA[min];
   MAX = MA[max];
  GetiMAArray2(PERIOD_M5,55,17,MODE_SMA,PRICE_CLOSE,day*1441,MA55);
   Get_XY(MA,MA55,RSI,XY,iY);
   ...
   Print("##### REGISTER DATA #####");
    ...
    Print("++++++++++++XY+++++++++++++");
      ...
      for(int y=0;y<ArraySize(iY);y++) {
         ...
         NormalizePriceOnly(X); //Normalize X
         double out = XY[i]; //Y
         Print(out);
         i = i+1; //NEXT X
...
void OnTick()
  ...
  if(fit==false) {
     ...
     double X[];
     int nbx=0;
      GetiRSIArray(351,PERIOD_M5,30,RSI);
      double MA55[];
      double MA[];
      GetiMAArray2(PERIOD_M5,55,17,MODE_SMA,PRICE_CLOSE,351,MA55);
      GetiMAArray2(PERIOD_M5,1,0,MODE_SMA,PRICE_CLOSE,351,MA);
      int posTotal = PositionsTotal();
       if(MA55[0]>=MA[0] && MA55[1]<MA[1]) {
         for(int x=0;x<=350;x++) {
            nbx = nbx + 1;
            ArrayResize(X,nbx);
            X[nbx-1] = MA[x];
         }
          NormalizePriceOnly(X);
         double out = ComputXYData("compute",X,0);
         Print(out);
         
         if(posTotal<=0 && out>0.6) {
            // Print("gain");
            BuyPosition(0.2);
         }
         posTotal = PositionsTotal();
         if(posTotal<=0 &&  out<0.4) {
            //Print("gain");
            SellPosition(0.2);
         }
      }
