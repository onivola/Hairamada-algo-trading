...
bool SimpleEnterSell(double check1) {
   if(check1==0){
...
bool simpleEnterBuy(double check1) {
...
bool EnterSellMiddle(double check1) {
   double bandsMiddle = iBands(1);
   double bandsup = iBands(100);
   double bandsdown = iBands(50);
   double price = GetTmpPrice();
   if(check1==0 && price>=bandsup){
      SellPosition();
      return true;
   }
   return false;
}
bool EnterSellUp(double check1) {
   double bandsMiddle = iBands(1);
   double bandsup = iBands(100);
   double bandsdown = iBands(50);
   double price = GetTmpPrice();
...
void OnTick(){
      ...
      double ema012 = iMA(0,PERIOD_M1,80,MODE_LWMA);
       double ema45 = iMA(20,PERIOD_M1,25,MODE_LWMA);
      double ema412 = iMA(5,PERIOD_M1,80,MODE_LWMA);
      /*
       double ema080 = iMA(0,PERIOD_M1,100,MODE_LWMA);
      double ema0160 = iMA(0,PERIOD_M1,200,MODE_LWMA);
       double ema180 = iMA(1,PERIOD_M1,100,MODE_LWMA);
      double ema1160 = iMA(1,PERIOD_M1,200,MODE_LWMA);*/
      
      //iMA(5,PERIOD_M1,10,MODE_LWMA);
     ...
     // int check1 = CheckEMA(PERIOD_M1,ema012,ema05,ema412,ema45);
       ...
       double bandsup = iBands(100);
       double bandsdown = iBands(50);
      int check1 = CheckEMA(PERIOD_M1,bandsMiddle,ema05,bandsMiddle,ema45);
      int check2 = CheckEMA(PERIOD_M1,bandsMiddle,ema012,bandsMiddle,ema412);
      //int check2 = CheckEMA(PERIOD_M1,ema0160,ema080,ema1160,ema180);
     /* int macd =  iMACD(PERIOD_M1,0);
      ...
      int posTotal = getSymbolPositionTotal(_Symbol);
       ...
       /*if(posTotal==0 && pos!=1 && price<(bandsMiddle+10)) { 
         
         if(simpleEnterBuy(check1)) {
            pos=1;
            BuyPosition();
     ...
     /* if(Profit()>=0.5 && pos==1) {
      ...
      if(Profit()<=-0.5 || Profit()>=50) {
       ...
       if(posTotal>=0 && pos==0) { 
         ...
         if(EnterSellUp(check1) && ema05>=(bandsup-10)) {
            pos=1;
            enter = price;
            sellSl = price+(price-bandsMiddle);
            SellPosition();
      ...
      if(Profit()>=1 && pos==1) {
       ...
       if((posTotal>=2 && price>=enter)) {
         close(_Symbol);
         pos=0;
      }
      if(price<=bandsMiddle || price>=sellSl) {
         close(_Symbol);
         pos=0;
      }*/
      /* if(posTotal>=0 && pos!=1) { 
          if(SimpleEnterSell(check1)) {
            pos=1;
            close(_Symbol);
       ...
       /* if(posTotal>=0 && pos2!=3) { 
          if(simpleEnterBuy(check2)) {
            pos2=3;
            close(_Symbol);
       ...
       if(posTotal>=0 && pos2!=4) { 
          if(SimpleEnterSell(check2)) {
                close(_Symbol);
            pos2=4;
       ...
       /*if(profit<=-0.6 || profit>=0.6) {
         close(_Symbol);
         pos=0;
      }*/
        /*if(posTotal>=0 && pos==0) { 
         
         if(EnterSellUp(check1) && ema05>=bandsup) {
            pos=1;
            SellPositionTPSL(0,price+(price-bandsMiddle));
            
         }
      }
      if(price<=bandsMiddle) {
         close(_Symbol);
         pos=0;
      }*/
      /* 
...
bool SellPositionTPSL(double TP,double SL) {
   CTrade trade;
   if(trade.Sell(0.2,_Symbol,TP,SL,0)){
      int code = (int)trade.ResultRetcode();
      ulong ticket = trade.ResultOrder();
      return true ;
   }
